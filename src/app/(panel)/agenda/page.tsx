...
export default async function AgendaPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const vets = await getActiveVeterinarians(session.clinicId);
  ...
  if (view === "dia") {
    ...
    content = <DayView date={date} timezone={timezone} slots={slots} vets={columns} appointments={appointments} />;
  } else {
    ...
    content = <WeekView weekDates={weekDates} timezone={timezone} appointments={appointments} showVet={!vetId} />;
  }
